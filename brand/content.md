# Portfolio content and information architecture

## Positioning

Primary: Senior Data Engineer. Secondary search terms: Data Platform Engineer
and Data Infrastructure Engineer. Berlin-based; open to relevant Germany and
European remote opportunities. <!-- C07–C23, C30–C35 -->

## Page order

1. Identity, role, Berlin location, and direct resume/contact actions.
2. Operating profile: pipelines, orchestration, cloud delivery, reliability.
3. Experience: exact employer titles and chronology.
4. Two evidence-backed work narratives, classified as professional experience.
5. Skills in demonstrated context.
6. Public historical projects, clearly separated from professional work.
7. Contact and same-origin resume route.

## Hero copy

**Eyebrow:** Berlin · Data Engineering

**Headline:** Data systems built for the work after launch.

**Summary:** I am Saurabh Shubham, a Data Engineer in Berlin. Across GROPYUS,
Sigmoid, and Amdocs, I have worked on production data flows, ETL pipelines, and
backend integrations using Python, orchestration tools, and cloud platforms.
<!-- C01, C07–C23 -->

## Work narratives

### Manufacturing data flows

**Classification:** Professional experience · GROPYUS
**Problem:** Production insight depends on data moving reliably between robotic
systems and enterprise platforms.
**Contribution:** Designed and optimised pipelines and data flows using Python,
Dagster, Airflow, dbt, DLT, and Azure.
**Validation/outcome boundary:** Candidate evidence supports production-insight
and automation context, but no public metric or internal architecture. Publish
no throughput, latency, uptime, topology, or confidential data. <!-- C12–C13 -->

### Commercial reporting pipelines

**Classification:** Professional experience · Sigmoid
**Problem:** Commercial reporting required repeatable ETL processing across
sales datasets.
**Contribution:** Built and maintained pipelines using Python, PySpark, Airflow,
Pandas, Terraform, Google Cloud, and AWS.
**Validation/outcome boundary:** Candidate evidence supports end-to-end reporting
work, not public scale or business-impact metrics. Client names stay private.
<!-- C17–C18 -->

## Public project note

Older public repositories show continued hands-on building in JavaScript and
Node.js. They are labelled historical projects, not current Data Engineering
case studies or production systems. <!-- C36, C38–C39 -->

## Exclusions

No claims about SampadAI, quantitative finance, agent frameworks, Supabase,
privacy hardening, promotions, team leadership, SLAs, scale, or business metrics.
No prohibited target titles. No client names. No copied reference-resume facts.
